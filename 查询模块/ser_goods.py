...
class SerGoods():
    ...
    def __init__(self, goInfSerMain, ID):
        ...
        self.root.title('库存查询')
        self.root.geometry('500x200+50+150')

        self.top_title = Label(self.root, text='查询单个货物请输入货物代码', bg='SkyBlue', font=('楷体', 20), width=70, height=1)
        self.top_title.pack()
        # 获取信息
        self.Id = []
        self.name = []
        ...
        self.buttonsr=Button(self.root, text="查看全部库存", font=tkFont.Font(size=16), command=self.new_row_infsr_all, width=20,
               height=2, fg='white', bg='gray').place(x=150, y=120)
        self.right_top_id_labelsrg = Label(text="货物代码：", font=('楷体', 15)).place(x=70, y=40)
        self.right_top_id_entrysrg = Entry(textvariable=self.var_Id, font=('楷体', 15)).place(x=170, y=40)
        self.right_top_button3srg = ttk.Button(text='查找', width=10, command=self.new_row_infsr_sig).place(x=160, y=80)
        self.right_top_button3srg = ttk.Button(text='返回', width=10, command=self.bf_goSerMainPage).place(x=300, y=80)
    ...
    def new_row_infsr_sig(self):#查询单个
        try:
            self.a.destroy()
        except:
            pass
        ...
        if  self.var_Id.get() != '':
            ...
            if results==():
                ...
                self.a = Tk()
                self.a.geometry('715x150+450+50')
    ...
    def new_row_infsr_all(self):#查询全部
        try:
            self.a.destroy()
        except:
            pass
        ...
        # messagebox.showinfo('提示！', '修改成功，请重新启动')
        self.a = Tk()
        self.a.geometry('715x700+450+50')
        b = TinUI(self.a, bg='white')
        b.pack(fill='both', expand=True)
        # b.add_table((0, 0),
        # data=(self.Id, self.name, self.type, self.position, self.num, self.danwei, self.supplyer))
