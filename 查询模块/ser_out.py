...
class SerOut():
    ...
    def __init__(self, goInfSerMain, ID):
        ...
        self.root.title('出库记录查询')
        self.root.geometry('500x200+50+150')

        self.top_title = Label(self.root, text='查询单个出库记录请输入出库单号', bg='SkyBlue', font=('楷体', 20), width=70, height=1)
        self.top_title.pack()
        # 获取信息
        self.Id = []
        self.name = []
        ...
        self.buttonouto=Button(self.root, text="查看全部记录", font=tkFont.Font(size=16), command=self.new_row_out_all, width=20,
               height=2, fg='white', bg='gray').place(x=150, y=120)
        self.right_top_id_labelout = Label(text="出库单号：", font=('楷体', 15)).place(x=70, y=40)
        self.right_top_id_entryout = Entry(textvariable=self.var_Id, font=('楷体', 15)).place(x=170, y=40)
        self.right_top_buttonout = ttk.Button(text='查找', width=10, command=self.new_row_out_sig).place(x=160, y=80)
        self.right_top_buttonout = ttk.Button(text='返回', width=10, command=self.bf_goSerMainPage).place(x=300, y=80)
    ...
    def new_row_out_sig(self):#查询单个
        ...
        if  self.var_Id.get() != '':
            ...
            sql = "SELECT * FROM 出库单 WHERE 出库单编号 = '%s'" % (self.var_Id.get())
            print(sql)
            self.data = []
            self.data.append(('出库单编号','货物代码','货物名称','货物类型','存放库位','出库总量','计量单位','出库单价','出库时间','供应商','经办人'))
